...
<ul>
<?php $_result=db('content')->field('content_id,title,pic,jumpurl,create_time')->where(" class_id in(".model("index/Catagory")->getSubClassId($classid).") and status =10 ")->order('sortid desc,content_id desc')->limit(($p-1)*12,12)->select();$count=db('content')->where(" class_id in(".model("index/Catagory")->getSubClassId($classid).") and status =10 ")->count();$Page=new \app\common\org\PageHtml($count,12,$info["filepath"],$p);$showpage=$Page->show();foreach($_result as $key=>$list):$list["url"]=getListUrl($list); $list["spic"]=getSpic($list); ?>
<div class='albumblock'>
<div class='inner'><a href='<?php echo $list['url']; ?>'><img src='<?php echo $list['pic']; ?>' /><div class='albumtitle'><?php echo $list['title']; ?></div></a></div></div>
<?php endforeach?>
</ul>
</div>
<div class="clearfix"></div>
