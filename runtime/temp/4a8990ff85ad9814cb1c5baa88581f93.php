<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:59:"D:\WWW\xhcms\home/../application/index\view\index\list.html";i:1556713711;s:53:"D:\WWW\xhcms\application\index\view\index\header.html";i:1556618863;s:51:"D:\WWW\xhcms\application\index\view\index\left.html";i:1556617792;s:51:"D:\WWW\xhcms\application\index\view\index\foot.html";i:1556600883;}*/ ?>
...
<body>
	<SCRIPT src="/qqkefu/HituxBlue/qq.htm"></SCRIPT>
...
<ul>
<?php $_result=db('content')->field('content_id,title,pic,jumpurl,create_time')->where(" class_id in(".model("index/Catagory")->getSubClassId($classid).") and status =10 ")->order('sortid desc,content_id desc')->limit(($p-1)*2,2)->select();$count=db('content')->where(" class_id in(".model("index/Catagory")->getSubClassId($classid).") and status =10 ")->count();$Page=new \app\common\org\PageHtml($count,2,$info["filepath"],$p);$showpage=$Page->show();foreach($_result as $key=>$list):$list["url"]=getListUrl($list); $list["spic"]=getSpic($list); ?>
<tr><td width='90%' class='fw_t'>·<a href='<?php echo $list['url']; ?>'><?php echo $list['title']; ?></a></td><td width='10%' class='fw_s'>[<?php echo date("Y-m-d",$list['create_time']); ?>]</td></tr>
<?php endforeach?>
</ul>
        <tr>
...
<div class="clearfix"></div>
	 <div class='t_page ColorLink'><?php echo $showpage; ?></div>
