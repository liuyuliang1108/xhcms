<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:72:"D:\phpStudy\WWW\xhcms\home/../application/admin\view\position\index.html";i:1556813246;s:67:"D:\phpStudy\WWW\xhcms\application\admin\view\common\_container.html";i:1556813246;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
				...
				<div class="row row-lg">
					<div class="col-sm-12">
						...
						<div id="CodeGoodsTableToolbar" role="group">
							<?php if(in_array('/admin/Position/add',session('admin.nodes')) || session('admin.role') == 1): ?>
							<button type="button" id="add" class="btn btn-primary button-margin" onclick="CodeGoods.add()">
								<i class="fa fa-plus"></i>&nbsp;添加
							</button>
							<?php endif; if(in_array('/admin/Position/update',session('admin.nodes')) || session('admin.role') == 1): ?>
							<button type="button" id="update" class="btn btn-primary button-margin" onclick="CodeGoods.update()">
								<i class="fa fa-edit"></i>&nbsp;修改
							</button>
							<?php endif; if(in_array('/admin/Position/delete',session('admin.nodes')) || session('admin.role') == 1): ?>
...
<script>
	...
	CodeGoods.initColumn = function () {
 		return [
 			...
 			{title: '编号', field: 'position_id', visible: true, align: 'center', valign: 'middle'},
 			...
 			{title: '操作', field: 'position_id', visible: true, align: 'center', valign: 'middle',formatter: 'CodeGoods.buttonFormatter'},
 		];
 	};
	...
	CodeGoods.add = function (value) {
		...
		var index = layer.open({type: 2,title: '创建数据',area: ['600px', '350px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Position/add'+url});
	...
	CodeGoods.update = function (value) {
		if(value){
			var index = layer.open({type: 2,title: '编辑数据',area: ['600px', '350px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Position/update/position_id/'+value});
		}else{
			if (this.check()) {
				var idx = '';

				$.each(CodeGoods.seItem, function() {
					idx += ',' + this.position_id;
				});
				...
				var index = layer.open({type: 2,title: '编辑数据',area: ['600px', '350px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Position/update/position_id/'+idx});
				this.layerIndex = index;
			}
		}
	}
	...
	$(function() {
		...
		var url = location.search;
		var table = new BSTable(CodeGoods.id, Feng.ctxPath+"/Position/index"+url,defaultColunms,20);
